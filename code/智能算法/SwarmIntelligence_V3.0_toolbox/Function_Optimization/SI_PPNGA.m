function [x,f,F] = SI_PPNGA(Options)
% PPNGA，伪并行小生境遗传算法
disp('--- 伪并行小生境遗传算法（Pseudo-Parallel Niche Genetic Algorithm,PPNGA） ---');
% 输入参数:
% Options - 优化参数设置
%     Options.dim - 优化函数的维数
...
for gen = 1:maxgen
    ...
    for j = 1:popnum_sub
        ...
        Population2 = CrossGa(Population2,fitness,Parmaters,maxmin,Lb,Ub,Pc);
        % 交叉操作
        % 输入参数:
        % Population - 子种群
        %     Population.X - 种群(行向量)
    ...
    if mod(gen,show)==0
        ...
        disp(['PPNGA, gen=',num2str(gen),', fval=',num2str(f,16),', x=',str]);
%         disp(['PPNGA, gen=',num2str(gen),', fval=',num2str(f,16)]);
    end
