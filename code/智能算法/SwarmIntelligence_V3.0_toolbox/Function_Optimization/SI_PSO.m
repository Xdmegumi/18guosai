function [x,f,F] = SI_PSO(Options)
% PSO，标准粒子群算法
disp('--- 标准粒子群算法（Particle Swarm optimization,PSO） ---');
% 输入参数:
% Options - 优化参数设置
%     Options.dim - 优化函数的维数
...
for gen = 1:maxgen
    ...
    if ispop==1
        ...
        for j = 1:popnum_sub
            ...
            Population2.V = Population1.V(J(:,j),:);
            Population2.Xp = Population1.Xp(J(:,j),:);
            Population2.Fp = Population1.Fp(J(:,j));
            
            
            Population2 = UpdatePso(Population2,fitness,Parmaters,maxmin,Lb,Ub,gen,maxgen,c1,c2,w1,w2);
            ...
            Population1.V(J(:,j),:) = Population2.V;
            Population1.Xp(J(:,j),:) = Population2.Xp;
            Population1.Fp(J(:,j)) = Population2.Fp;
            
        end          
                
    else
        
        Population1 = UpdatePso(Population1,fitness,Parmaters,maxmin,Lb,Ub,gen,maxgen,c1,c2,w1,w2);
        % 种群更新
        % 输入参数：
        % Population - 种群
        %     Population.X - 种群(行向量)
    ...
    if mod(gen,show)==0
        ...
        disp(['PSO, gen=',num2str(gen),', fval=',num2str(f,16),', x=',str]);
%         disp(['PSO, gen=',num2str(gen),', fval=',num2str(f,16)]);
    end
