function [x,f,F] = SI_ABC(Options)
% ABC，蜂群算法（Artificial Bee Colony,ABC）
disp('--- 蜂群算法（Artificial Bee Colony,ABC） ---');
% Options - 优化参数设置
%     Options.dim - 优化函数的维数
...
for gen = 1:maxgen
    ...
    if ispop==1
        ...
        for j = 1:popnum_sub
            ...
            [Population2] = UpdateAbc(Population2,fitness,Parmaters,maxmin,Lb,Ub);
            % 种群更新
            % 输入参数：
            % Population - 种群
            %     Population.X - 种群(行向量)
        ...
        [Population1] = UpdateAbc(Population1,fitness,Parmaters,maxmin,Lb,Ub);
    ...
    F(gen) = f;
    F_Array(:,gen) = Population1.F;
    
    %---------------------------------------
    
    if gen>=limit
        tmp1 = Population1.F - F_Array(:,gen-limit+1);
        J = find(tmp1==0);
        if ~isempty(J)
            tmp2 = Initialize(fitness,Parmaters,maxmin,Lb,Ub,length(J));
            % 种群初始化
            % 输入参数:
            % fitness - 优化函数
            % Parmaters - 函数参数   
            ...
            Population1.X(J,:) = tmp2.X;
            Population1.F(J) = tmp2.F;
        end
    end    
    ...
    if mod(gen,show)==0
        ...
        disp(['ABC, gen=',num2str(gen),', fval=',num2str(f,16),', x=',str]);
%         disp(['ABC, gen=',num2str(gen),', fval=',num2str(f,16)]);
    end
