function [x,f] = SI_FMINCON(Options)
% FMINCON优化
disp('------------ FMINCON优化 ------------');
% 输入参数:
% Options - 优化参数设置
...
try
    options = optimset('Algorithm','interior-point','LargeScale','on','Display','off','TolX',1e-16,'TolFun',1e-16,'TolCon',1e-16,'MaxIter',500,'MaxFunEvals',1e+5);
    [x,f] = fmincon(@(X) MyFun(X,fitness,Parmaters,maxmin),X0,A,b,Aeq,beq,LB,UB,nonlcon,options)
    
%     options = optimset('Algorithm','active-set','LargeScale','on','Display','iter','TolX',1e-40,'TolFun',1e-40,'TolCon',1e-40,'MaxIter',maxgen,'MaxFunEvals',5e+6);
%     [x,f] = fmincon(@(X) MyFun(X,fitness,Parmaters,maxmin),X0,A,b,Aeq,beq,LB,UB,nonlcon,options);
%     
%     options = optimset('Algorithm','trust-region-reflective','LargeScale','on','Display','iter','TolX',1e-40,'TolFun',1e-40,'TolCon',1e-40,'MaxIter',maxgen,'MaxFunEvals',5e+6);
%     [x,f] = fmincon(@(X) MyFun(X,fitness,Parmaters,maxmin),X0,A,b,Aeq,beq,LB,UB,nonlcon,options);
%     
%     options = optimset('Algorithm','sqp','LargeScale','on','Display','iter','TolX',1e-40,'TolFun',1e-40,'TolCon',1e-40,'MaxIter',maxgen,'MaxFunEvals',5e+6);
%     [x,f] = fmincon(@(X) MyFun(X,fitness,Parmaters,maxmin),X0,A,b,Aeq,beq,LB,UB,nonlcon,options);
catch
    options = optimset('LargeScale','off','Display','off','TolX',1e-16,'TolFun',1e-16,'TolCon',1e-16,'MaxIter',500,'MaxFunEvals',1e+5);
    [x,f] = fmincon(@(X) MyFun(X,fitness,Parmaters,maxmin),X0,A,b,Aeq,beq,LB,UB,nonlcon,options)
    
%     options = optimset('LargeScale','on','Display','iter','TolX',1e-40,'TolFun',1e-40,'TolCon',1e-40,'MaxIter',maxgen,'MaxFunEvals',5e+6);
%     [x,f] = fmincon(@(X) MyFun(X,fitness,Parmaters,maxmin),X0,A,b,Aeq,beq,LB,UB,nonlcon,options);
end
