function [x,f,F] = SI_MSFLA(Options)
%  MSFLA，改进混合蛙跳算法
disp('--- 改进混合蛙跳算法（Modified Shuffled Frog-leaping Alogrihtm,MSFLA） ---');
% 输入参数:
% Options - 优化参数设置
%     Options.dim - 优化函数的维数
...
for gen = 1:maxgen
    ...
    for j = 1:popnum_sub
        ...
        [Population2] = CrossMsfla(Population2,fitness,Parmaters,maxmin,Lb,Ub,x,f);
        % 更新操作
        % 输入参数:
        % Population - 子种群
        %     Population.X - 种群(行向量)
        ...
        [Population2] = Mutate(Population2,fitness,Parmaters,maxmin,Lb,Ub,Pm);
        % 变异操作
        % 输入参数:
        % Population - 子种群
        %     Population.X - 种群(行向量)
    ...
    if mod(gen,show)==0
        ...
        disp(['MSFLA, gen=',num2str(gen),', fval=',num2str(f,16),', x=',str]);
%         disp(['MSFLA, gen=',num2str(gen),', fval=',num2str(f,16)]);
    end
