function [x,f,F] = SI_DEb2(Options)
% DEb2，标准差分算法(DE/best/2/bin)
disp('--- 标准差分算法(DE/best/2/bin)（Differential Evolution Algorithms,DE） ---');
% Options - 优化参数设置
%     Options.dim - 优化函数的维数
...
for gen = 1:maxgen
    ...
    if ispop==1
        ...
        for j = 1:popnum_sub
            ...
            Population2 = UpdateDEb2(Population2,fitness,Parmaters,maxmin,Lb,Ub,K,CR);
            % 种群更新
            % 输入参数：
            % Population - 种群
            %     Population.X - 种群(行向量)
        ...
        Population1 = UpdateDEb2(Population1,fitness,Parmaters,maxmin,Lb,Ub,K,CR);
    ...
    if mod(gen,show)==0
        ...
        disp(['DEb2, gen=',num2str(gen),', fval=',num2str(f,16),', x=',str]);
%         disp(['DEb2, gen=',num2str(gen),', fval=',num2str(f,16)]);
    end
