function [x,f,F] = SI_AF_SFLA(Options)
...
for gen = 1:maxgen
    ...
    if ispop==1
        ...
        for j = 1:popnum_sub
            ...
            [Neighbor] = GetNeighbor(Population2);
            [Population2] = CrossAfSfla(Population2,Neighbor,fitness,Parmaters,maxmin,Lb,Ub);
        ...
        [Neighbor] = GetNeighbor(Population1);
        % 计算个体邻域
        % 输入参数:
        % Population - 种群
        %     Population.X - 种群(行向量)
        ...
        [Population1] = CrossAfSfla(Population1,Neighbor,fitness,Parmaters,maxmin,Lb,Ub);
        % 鱼群交叉操作
        % 输入参数：
        % Population1 - 初始种群
        %     Population.X - 种群(行向量)
    ...
    if mod(gen,show)==0
        ...
        disp(['AF_SFLA, gen=',num2str(gen),', fval=',num2str(f,16),', x=',str]);
%         disp(['AF_SFLA, gen=',num2str(gen),', fval=',num2str(f,16)]);
    end
