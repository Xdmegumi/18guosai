...
c = 4;              % 聚类数
tmax = 20;          % 最大迭代次数
b = 1.2;              % 大于1的实数,b=(1,2)非线性扩张,b=(2,+inf)非线性压缩
g = 0.01;           % Jf变化门限      
...
while k<tmax
    ...
    k = k+1;    
    [T,tmp,jf,M] = FuzzyKMeansImproved(X,M,b);   % 输出聚类结果和代价函数收敛曲线
    Jf(k) = jf;                                  % 代价函数赋值
...
% [T,N,jf,M2] = FuzzyKMeansImproved(X,M,b)
% 改进的模糊 K-Means 聚类
% 参考文献: % 边肇祺 编著. 模式识别[M]. 北京:清华大学出版社. 1999. p281
% 输入参数:
% X  - 样本点,每一列一个点
% M - 聚类中心,每一列一个点(老的)
% b  - 参数b
...
title(['Improved Fuzzy K-Mean Clustering, c = ',num2str(c)]); hold off;
