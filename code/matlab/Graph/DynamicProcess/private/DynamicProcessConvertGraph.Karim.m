...
LinkedNodeIDs = unique(reshape(Graph.Data(:,1:2), 2*size(Graph.Data,1),1));
if ~isempty(setdiff(LinkedNodeIDs, double(DynamicProcessGraph.NodeIDs))), error('%d nodes are not defined in NodeIDs parameter',numel(setdiff(LinkedNodeIDs, double(DynamicProcessGraph.NodeIDs)))); end
clear ('LinkedNodeIDs');

[y x] = hist(Graph.Data(:,2),unique(Graph.Data(:,2)));
[~, ai, bi] = intersect(x,double(DynamicProcessGraph.NodeIDs));
